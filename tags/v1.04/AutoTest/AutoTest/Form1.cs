...
*/
        ...
        public void readSch()
        {
            ...
            {
                MessageBox.Show("File does not exist, please check file path correctness.", "File Open Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (TextLine != "")
            {
                ...
                {
                    button1.Enabled = true;
                    MessageBox.Show("This csv file is older version. If you want to use it, it will not operate properly resulting in delay time.", "File Open Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                if (j != 8 && j != 10)
                {
                    button1.Enabled = false;
                    button2.Enabled = false;
                    MessageBox.Show("This csv file format error, please check csv file format.", "File Format Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
                    ...
                    for (int i = 0; i < this.DataGridView1.Rows.Count - 1; i++)
                    {
                        ...
                        if (this.DataGridView1.Rows[i].Cells[0].Value.ToString() == "_cmd")
                        {
                            ...
                            if (DataGridView1.Rows[i].Cells[3].Value.ToString() == "_shot")
                            {
                                ...
                                {
                                    MessageBox.Show("This status can't open camera, please check camera connect status.", "Camera Open Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                        ...
                        else
                        {
                            ...
                            {
                                MessageBox.Show("This status can't open redrat, please check redrat connect status.", "Redrat Open Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
        ...
        protected void savevideo()
        {
            ...
            string t = fName + "\\" + "_pvr" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + label1.Text + ".wmv";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Autobox does not exist, please check autobox and computer connect status.", "Autobox Open Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                status = false;
            }
            ...
            if (status == true)
            {
                ...
                if (jbutton1 == true)
                {
                    ...
                    MessageBox.Show("Please Wait for Delay Time End.", "Waiting Loop Message", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    if (ini12.INIRead(Application.StartupPath + "\\Config.ini", "IR Device", "value", "") == "2")
