...
*/
            ...
            // 先判斷VID, PID是否存在
            if (VIDTextBox.Text == "" & PIDTextBox.Text == "")
            {
            ...
            else
            {
                if (VIDTextBox.Text == "")
                {
                    MessageBox.Show("VID must exist !", "UserDefine Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    open = false;
                }
                else if (PIDTextBox.Text == "")
                {
                    MessageBox.Show("PID must exist !", "UserDefine Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    PIDTextBox.BackColor = System.Drawing.Color.Yellow;
                    open = false;
                }
                else
                {
                    ini12.INIWrite(sPath, "Camera", "Userdefined", "1");
                    ini12.INIWrite(sPath, "Camera", "VID", VIDTextBox.Text.Trim());
                    ini12.INIWrite(sPath, "Camera", "PID", PIDTextBox.Text.Trim());
                }
            }
...
*/
            ...
            {
                MessageBox.Show("Save Log Path does not exist", "Path Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                open = false;
            }
            else
            {
