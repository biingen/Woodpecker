...
namespace USB_CAN2C
{
    public class CAN_Data
    {
        ...
        unsafe public void TransmitData(string ID, string Data)
        {
            ...
            sendobj.ID = System.Convert.ToUInt16("0x" + ID, 16);
        ...
        public const int ReadCanbusDiagnosticCodesByStatus_MaxNumberOfCanbus = 255;
        private byte can_id;
        ...
        private List<byte> GenerateQueuedResponseData_CAN()
        {
            List<byte> ret_list = new List<byte>();
            List<byte> data_of_can_list = new List<byte>();
            Byte CAN_no = 0;

            while ((CAN_Write_Data_Queue.Count > 0) && (CAN_no < ReadCanbusDiagnosticCodesByStatus_MaxNumberOfCanbus))
            {
                CAN_Data this_can_ctl = CAN_Write_Data_Queue.Dequeue();
                ret_list.AddRange(this_can_ctl.ToByteList());
                CAN_no++;
            }
            data_of_can_list.Add(CAN_no);
            data_of_can_list.AddRange(ret_list);
            return data_of_can_list;
        }
    }
}
