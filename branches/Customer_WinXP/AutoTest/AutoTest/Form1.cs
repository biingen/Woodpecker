...
*/
        ...
        {
            // 寫入Camera啟動
            if (ini12.INIRead(Application.StartupPath + "\\Config.ini", "Device", "Name", "") != "")
            {
            ...
            for (int j = 1; j < Global.Schedule_Loop + 1; j++)
            {
                ...
                lock (this)
                {
                    for (int i = 0; i < DataGridView1.Rows.Count - 1; i++)
                    {
						...
						else if (DataGridView1.Rows[i].Cells[0].Value.ToString() == "_MonkeyTest")
                        {
                            Add_ons MonkeyTest_All = new Add_ons();
                            MonkeyTest_All.MonkeyTest();
                        }

                        #region Philips TV Hotel Mode command 控制
                        else if (DataGridView1.Rows[i].Cells[0].Value.ToString() == "_Hotel")
                        {
                            string str = DataGridView1.Rows[i].Cells[5].Value.ToString();
...
*/
        ...
        void cmb_DropDown(object sender, EventArgs e)
        {
            ...
            DataGridViewComboBoxColumn c =
                this.DataGridView1.Columns[0] as DataGridViewComboBoxColumn;
            if (c != null)
            {
